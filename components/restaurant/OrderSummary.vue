        ...
        <AtomTheCard rounded="xl" shadow="none" class="summary relative bg-gray-100 p-6 min-w-[16rem] max-w-full">
            <div class="w-full flex flex-col gap-2">
                <p aria-label="" class="flex justify-between font-normal text-sm text-gray-400">
                    Subtotal <span class="font-medium text-gray-800">{{ formatted(subTotal) }}</span>
                </p>
                <p aria-label="" class="flex justify-between font-normal text-sm text-gray-400">
                    Tax(10%) <span class="font-medium text-gray-800">{{ formatted(calculateTax) }}</span>
                </p>
            </div>
            <div class="line"></div>
        </AtomTheCard>
...
.line {
    ...
    width: 100%;
    height: 0.125rem;
    background: repeating-linear-gradient(to right, transparent, transparent 10px, #d1d5db 10px, #d1d5db 20px);
}
</style>
