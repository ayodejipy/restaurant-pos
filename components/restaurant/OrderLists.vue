...
<template>
	<div class="flex-1 bg-gray-50 py-6 overflow-hidden">
		...
		<div v-if="orders?.length == 0" class="text-sm font-medium">List is currently empty. Any order you process will be displayed here.</div>
		<div v-else class="flex gap-6 pb-4 overflow-x-auto no-scrollbar">
			<RestaurantOrderCard v-for="order in orders" :key="order.customer_name" :order="order" />
		</div>
	</div>
</template>
